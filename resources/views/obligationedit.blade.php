...
@section('content')
        ...
        <div class="container">
            <div class="content">
                ...
                <div class="padding">
                     @if($suppObligation=='ok') <a href="/obligation/destroy/{{ $results->id }}" class="btn btn-danger pull-left" style="margin-right: 3px;">Supprimer</a><BR><BR><BR> @endif
                </div>
                    ...
                    @endforeach
                    {!! Form::open(array('url' => '/obligation/update')) !!}
                        ...
                        {!! Form::submit('Enregistrer', 
                          array('class'=>'btn btn-primary')) !!} @if($suppObligation=='ok') <a href="/obligation/destroy/{{ $results->id }}" class="btn btn-danger pull-left" style="margin-right: 3px;" onclick="if(!confirm('Etes-vous sûr de vouloire supprimer cette obligation ?')){return false;};">Supprimer</a> @endif ou <a href="/obligation/list/{{ $results->domaine_id }}">Retour</a>
                ...
                <div class="table-responsive">
                    <table class="table table-hover" style="table-layout:fixed;" >
                        ...
                        <tbody>
                            ...
                            <TR><TD>{{ $obligationdetail->frequence}}</TD><TD>{{ $obligationdetail->frequence_type}}</TD><TD>{{ $obligationdetail->txt_1}}</TD><TD>{{ $obligationdetail->txt_2}}</TD><TD> @if($detailSupp[$obligationdetail->id]=='ok') <a href='/obligationdetail/destroy/{{ $obligationdetail->id }}' class="btn btn-danger pull-left" style="margin-right: 3px;">Supprimer</a> @endif</TD></TR>
                            @endforeach
                            <TR><TD bgcolor="#FFFFFF">
                                <form method="post" action="/obligation/adddetail" name="obligationForm"><input type="hidden" name="_token" value="{{{ csrf_token() }}}" /><input type="hidden" name="obligation_id" value="{{ $results->id}}">
